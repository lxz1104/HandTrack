...
void test3DView() {
    ...
    // flag
    int key = 0;
    boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> ptr;
    while (key != 27) {
        cv::imshow("XYZMap", camera->getXYZMap());
        ptr = util::toPointCloud<pcl::PointXYZ>(camera->getXYZMap());
        Visualizer::visualizeCloud(ptr);
...
//    std::chrono::high_resolution_clock timer = std::chrono::high_resolution_clock();
    ...
    while (true) {
        ...
        if (currFrame % FPS_CYCLE_FRAMES == 0) {
            ...
            currFPS = (float) FPS_CYCLE_FRAMES * 1000.0f /
                      std::chrono::duration_cast<ms>(now - currCycleStartTime).count();
