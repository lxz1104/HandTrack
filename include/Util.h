...
namespace ht {
    ...
    namespace util {
        ...
        */
        ...
        float
        pointLineDistance(const cv::Point_<Param_T> &p, const cv::Point_<Param_T> &a, const cv::Point_<Param_T> &b,
                          int cv_norm_type = cv::NORM_L2SQR);
        ...
        template<class Param_T>
        Param_T
        pointLineDistance(const cv::Vec<Param_T, 3> &p, const cv::Vec<Param_T, 3> &a, const cv::Vec<Param_T, 3> &b,
                          int cv_norm_type = cv::NORM_L2SQR);
         ...
         */
        template<class Param_T>
        float pointLineSegmentDistance(const cv::Point_<Param_T> &p, const cv::Point_<Param_T> &a,
                                       const cv::Point_<Param_T> &b, int cv_norm_type = cv::NORM_L2SQR);
         ...
         */
        template<class Param_T>
        Param_T pointLineSegmentDistance(const cv::Vec<Param_T, 3> &p, const cv::Vec<Param_T, 3> &a,
                                         const cv::Vec<Param_T, 3> &b, int cv_norm_type = cv::NORM_L2SQR);
        ...
        */
         ...
         */
        double angleBetween3DVec(Vec3f a, Vec3f b, const Vec3f &center = Vec3f(0, 0, 0));
         ...
         */
        bool pointOnEdge(cv::Size size, Point2i pt,
         ...
         */
        bool pointOnEdge(cv::Rect rect, Point2i pt,
                         int margin_tb = 30, int margin_lr = 30);
        ...
        */
        ...
        class PointComparer {
        public:
            explicit PointComparer(bool reverse = false, bool compare_y_then_x = false)
        ...
        Eigen::Vector3d cloudCenter(const boost::shared_ptr<pcl::PointCloud<Point_T>>
            ...
            return
                    Eigen::Vector3d(avgPos4.x(), avgPos4.y(), avgPos4.z());
        }
 ...
 */
        template<class T>
        boost::shared_ptr<pcl::PointCloud<T> >
        toPointCloud(const cv::Mat &xyz_map,
                     bool flip_z = true, bool flip_y = true, int step = 1) {
