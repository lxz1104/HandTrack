...
namespace ht {
    ...
    namespace util {
		...
		double getAngelOfTwoVector(const Point2i& pt1, const Point2i& pt2, const Point2i& pt3, const Point2i& pt4)
		{
			...
			double dy1 = pt2.y - pt2.y;
			double dx2 = pt4.x - pt3.x;
			double dy2 = pt4.y - pt3.y;
			double angle1 = atan2(dy1, dx1);
			double angle2 = atan2(dy2, dx2);
			angle1 = int(angle1 * 180 / CV_PI);
			angle2 = int(angle2 * 180 / CV_PI);
			if (angle1 * angle2 > 0)
			{
...
#ifdef _OPENMP
			const uint threads_num = r_upper > 12 ? 12 : r_upper;
#pragma omp parallel for num_threads(threads_num)  \
		 default(none) shared(average,count,xyzMap)
#endif // _OPENMP
        ...
        void removePlane(const cv::Mat & ref_cloud, cv::Mat & image, const Vec3f & plane_equation,
            double threshold, cv::Mat * mask, uchar mask_color)
        {
            //const Vec3f * refPtr;
            // T * imgPtr;
            uchar * maskPtr = nullptr;

#ifdef _OPENMP
			const uint threads_num = ref_cloud.rows > 12 ? 12 : ref_cloud.rows;
#pragma omp parallel for num_threads(threads_num)  \
		 default(none) shared(image,mask,mask_color)
#endif // _OPENMP
            for (int row = 0; row < ref_cloud.rows; ++row) {
				const Vec3f* refPtr = ref_cloud.ptr<Vec3f>(row);
				T* imgPtr = image.ptr<T>(row);
        ...
        template void removePlane<int>(const cv::Mat & ref_cloud, cv::Mat & image, const Vec3f & plane_equation, double threshold, cv::Mat * mask, uchar mask_color);
        template void removePlane<float>(const cv::Mat & ref_cloud, cv::Mat & image, const Vec3f & plane_equation, double threshold, cv::Mat * mask, uchar mask_color);
        template void removePlane<Vec3f>(const cv::Mat & ref_cloud, cv::Mat & image, const Vec3f & plane_equation, double threshold, cv::Mat * mask, uchar mask_color);
            ...
            std::vector<Point2i> * output_aux, int num_data_pts)
        {
            ...
            const int NUM_OUTPUT_PTS = int(num_data_pts * (1.0 - thresh));
        ...
        static inline float getTriangleArea(Vec3f a, Vec3f b, Vec3f c)
        {
            ...
            Vec3f cross(v0[1] * v1[2] - v1[1] * v0[2],
                v0[2] * v1[0] - v1[2] * v0[0],
                v0[0] * v1[1] - v1[0] * v0[1]);
            return static_cast<float>(magnitude(cross) / 2.0f);
        }
        ...
        double surfaceArea(const cv::Size & frame_size,
            ...
            for (size_t i = 0; i < cluster_size; ++i) {
                if (!i || points_ij[i].y > points_ij[i - 1].y)
                    rowStart.push_back(static_cast<int>(i));
            }
            ...
            for (size_t i = 0; i < rowStart.size() - 2; ++i) {
				size_t nx = rowStart[i + 1];

                for (size_t j = rowStart[i]; j < rowStart[i + 1] - 1; ++j) {
					size_t idx = j - rowStart[i];
            ...
            for (int r = 0; r < size.height; r++)
            {
                for (int c = 0; c < size.width; c++)
                {
                    ...
                    if (validPoint[0] && validPoint[1]) {
                        ...
                        surfArea += sqrt(fabs(s * (s - dist1) * (s - dist2) * (s - dist3)));
                    }
...
#ifdef _OPENMP
			const uint threads_num = (int)points.size() > 12 ? 12 : (int)points.size();
#pragma omp parallel num_threads(threads_num) \
			default(none) shared(a,b,maxd)
#endif // _OPENMP
            for (int i = 0; i < 2; ++i) {
#ifdef _OPENMP
#pragma omp parallel for
                ...
                for (int j = 0; j < (int)points.size(); ++j) {
                    double curd = ht::util::norm(points[j] - points[a]);
                    if (curd > maxd) {
                        b = j;
                        maxd = curd;
                    }
        ...
        ***/
            ...
            int inv1, int inv2, float inv2_thresh, cv::Mat * color, bool cosine)
        {
            ...
            const size_t R = xyz_map.rows, C = xyz_map.cols;
            ...
            if (cosine) {
                thresh *= thresh;	//使用距离平方保存计算过程	
                max_distance2 *= max_distance2; //对于Interval2
            }

			// 将种子添加到堆栈
            stk[0] = seed;

            size_t stkSize = 1, total = 0, nNext;

			//存储下一个点
            std::array<Point2i, 4> nextPts;

            Point2i pt;
            const Vec3f * xyzPtr = nullptr;
            Vec3f * oPtr = nullptr;
            uchar * visPtr = nullptr;
            bool sw;
        ...
        {
            angle += 2.5 * PI;
            return cv::normalize(cv::Vec2f(cosf(angle), sinf(angle)));
        }
        ...
        double magnitude(cv::Vec<T, n> pt) {
            double sm = 0;
            for (int i = 0; i < n; ++i) sm += double(pt[i]) * pt[i];
            return sqrt(sm);
        }
        ...
        double norm(const cv::Point3_<Param_T> & pt, int cv_norm_type) {
            if (cv_norm_type == cv::NORM_L2SQR) {
                return double(pt.x) * pt.x + double(pt.y) * pt.y + double(pt.z) * pt.z;
            }
            else if (cv_norm_type == cv::NORM_L2) {
                return magnitude(pt);
            }
        ...
        void radixSortPoints(std::vector<Point2i> & points, int wid, int hi, int num_pts,
            ...
            if (buckets == nullptr || lastWid != wid || lastHi != hi) {
				...
				// 为buckets永久分配内存，以提高效率
                buckets = new int[size_t(wid) * hi];
        ...
        Point2f largestInscribedCircle(const std::vector<Point2i> & contour,
			...
			// 1. 构造（下采样）轮廓点的Voronoi图
            typedef boost::polygon::point_data<int> xpoint;
            std::vector<xpoint> xpoints;

            size_t contour_step = std::max(1, (int)contour.size() / 100);
