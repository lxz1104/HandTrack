            ...
            if (cosangle > 0) {
                ...
                if (dist0 < dist1) {
                    ...
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i], fingersIJSort[i], cv::FONT_HERSHEY_COMPLEX, 0.5,
                                    cv::Scalar(0, 255, 255));
                    }
                }
                //手心
                if (dist0 > dist1) {
                    ...
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i], fingersIJSort[fingersIJSort.size() - 1 - i],
        ...
        if (fingersXYZSort.size() == 4 && defectsXYZSort.size() == 4) {
            ...
            if (cosangle > 0) {
                ...
                if (dist0 > dist1) {
                    cv::putText(output, "L_HB", center, cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i + 1], fingersIJSort[i], cv::FONT_HERSHEY_COMPLEX, 0.5,
                                    cv::Scalar(0, 255, 255));
                    }
                }
                //手心
                if (dist0 < dist1) {
                    cv::putText(output, "L_HP", center, cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i + 1], fingersIJSort[fingersIJSort.size() - 1 - i],
                                    cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    }
                }
            }
        ...
        if (fingersXYZSort.size() == 3 && defectsXYZSort.size() == 3) {
            ...
            if (cosangle > 0) {
                ...
                if (dist0 > dist1) {
                    cv::putText(output, "L_HB", center, cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i + 2], fingersIJSort[i], cv::FONT_HERSHEY_COMPLEX, 0.5,
                                    cv::Scalar(0, 255, 255));
                    }
                }
                //手心
                if (dist0 < dist1) {
                    cv::putText(output, "L_HP", center, cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    for (size_t i = 0; i < fingersIJSort.size(); i++) {
                        cv::putText(output, fingersname[i + 2], fingersIJSort[fingersIJSort.size() - 1 - i],
                                    cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 255));
                    }
                }
            }
